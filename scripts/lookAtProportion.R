...
library(binom)
library(lme4)
setwd("C:/Users/s03an7/Documents/GitHub/DeadlineExp/DeadlineExpFromGitHub/data")
...
aggData = (filter(fixdat, 
  ...
  fixNum>1, 
  targSide=="absent") 
  %>% group_by(subj,order,fixNum, version)
    %>% summarise(
...
plt = plt + scale_x_continuous(name="fixation number", breaks=c(2,3,4,5,6)) + annotate(geom="text", x=10, y=10, label="Scatter plot",colour="red")
...
ggsave("C:/Users/s03an7/Documents/GitHub/DeadlineExp/DeadlineExpFromGitHub/plots/proportionIndividual.pdf", width=6, height=3)
ggsave("plots/FixXsideByFixNumAndSubjExCentralInaccurateIncluded.jpg", width=6, height=3)
