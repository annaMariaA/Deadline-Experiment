...
```{r exp1modelprior, cache=TRUE, echo = TRUE, message = FALSE}
prior_model <- brm(
	data = d_strat,
...
dat_rt_acc %>% filter(cd %in% c("flat", "reward")) %>%
	...
	summarise(accuracy = mean(acc), .groups = 'drop') %>%
	ggplot(aes(x = bk, y = accuracy, fill = cd)) +
	geom_boxplot(alpha = 0.25) +
	facet_wrap(~ targ_side) +
	theme_bw() +
...
d_pwr_reward  %>% group_by(cd) %>% summarise(mean_ph = mean(prop_hetero))
```
...
```{r timecourse-posterior-drop-cd, cache=TRUE, message=FALSE}
...
```{r timecourse-posterior-drop-t, cache=TRUE, echo = TRUE, message=FALSE}
...
sessionInfo()
```
correlation between strategy and RT deadline
```{r}
...
```
correlation between strategy and RT reward
```{r}
#strategy in the flat condition only, target absent trials
dat_fix %>% left_join(dat_rt_acc, 
                      by = c("observer", "cd", "bk", "t")) %>% 
	filter(
		cd %in% c("flat"), 
