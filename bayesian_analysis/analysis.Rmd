...
dat_rt_acc %>% 
  filter(condition %in% c("flat", "reward")) %>%
  select(observer, bk_ = "block", cd_ = "condition", acc) %>%
  group_by(observer, bk_, cd_) %>%
  summarise(accuracy = mean(acc), .groups = "drop") -> d_lmer_acc

model_acc <- lmer(
  accuracy ~ bk_ * cd_ + (1|observer), 
  data =  d_lmer_acc)  
...
dat_rt_acc %>% 
  filter(condition %in% c("flat", "reward"), acc == 1) %>%
  select(observer, bk_ = "block", cd_ = "condition", rt) %>%
  group_by(observer, bk_, cd_) %>%
  summarise(median_rt = median(rt), .groups = "drop") -> d_lmer_rt

model_rt <- lmer(
  median_rt ~ bk_ * cd_ + (1|observer), 
  data =  d_lmer_rt)  
...
d_strat %>%
  select(observer,  bk_ = "block", cd_ = "condition", t, prop_hetero) %>%
   group_by(observer, bk_, cd_) %>%
  summarise(median_search_ef = median(prop_hetero), .groups = "drop") -> d_lmer_se

model_se <- lmer(
  median_search_ef ~ bk_ * cd_ + (1|observer), 
  data =  d_lmer_se)  
